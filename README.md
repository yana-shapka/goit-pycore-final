...
```
   ...
   Or directly:
```bash
   python3 -m personal_assistant.main
```
...
- **Phone:** must be exactly 10 digits (e.g. `0971234567`)
- **Email:** must be valid format (e.g. `name@example.com`)
- **Birthday:** must be in `DD.MM.YYYY` format (e.g. `25.12.1990`)

---

## Data Storage

All data is saved automatically on exit to:
- `addressbook.pkl` — contacts
- `notebook.pkl` — notes

Both files are restored automatically on next launch.
...
├── personal_assistant/
│   ├── __init__.py
│   ├── models.py       # Field, Record, AddressBook classes
